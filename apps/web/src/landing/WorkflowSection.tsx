...
export const WorkflowSection: React.FC = () => {
  ...
  const WORKFLOW_STEPS = [
    {
      step: '01',
      title: t('workflow.step1_title', 'Profile Your Business'),
      desc: t('workflow.step1_desc', 'Enter your business activity, project scale, state, and location. Our system instantly categorizes your industry classification.'),
      badge: 'Step 1',
      icon: (
    ...
    {
      step: '02',
      title: t('workflow.step2_title', 'AI Regulatory Engine Maps Norms'),
      desc: t('workflow.step2_desc', 'ApprovalIQ analyzes central, state, and local environmental, labor, fire, and industrial statutes to generate your bespoke compliance matrix.'),
      badge: 'Step 2',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    {
      step: '03',
      title: t('workflow.step3_title', 'Pre-Audit & Document Prep'),
      desc: t('workflow.step3_desc', 'Automated checklists, affidavit templates, and file validation ensure every application is error-free prior to formal submission.'),
      badge: 'Step 3',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    {
      step: '04',
      title: t('workflow.step4_title', 'Track End-to-End Clearance'),
      desc: t('workflow.step4_desc', 'Monitor department reviews, statutory timelines, queries, and license grants via a single unified dashboard.'),
      badge: 'Step 4',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto">
          <div className="text-xs font-bold uppercase tracking-widest text-brand-primary-600 mb-2">
            {t('workflow.tag', 'INTELLIGENT COMPLIANCE PIPELINE')}
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-900 tracking-tight">
            {t('workflow.title', 'How ApprovalIQ Streamlines Your Approvals')}
          </h2>
          <p className="mt-4 text-base sm:text-lg text-slate-600">
            {t('workflow.subtitle', 'From initial business registration to final statutory clearance in 4 structured steps.')}
          </p>
        </div>
          ...
          {WORKFLOW_STEPS.map((step) => (
            ...
            >
              ...
              <div className="mt-6 pt-4 border-t border-slate-100 flex items-center text-xs font-semibold text-brand-primary-600 group-hover:text-brand-primary-700">
                <span>{t('workflow.explore_step', 'Explore step')}</span>
        ...
        <div className="mt-16 bg-gradient-to-r from-[#0b1329] via-[#0f172a] to-[#1e1b4b] rounded-3xl p-8 sm:p-10 text-white shadow-2xl border border-indigo-900/50 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="max-w-2xl">
            <h3 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-white">
              {t('workflow.ready_title', 'Ready to automate your regulatory roadmap?')}
            </h3>
            <p className="mt-2 text-slate-300 text-sm sm:text-base">
              {t('workflow.ready_sub', 'Join Indian entrepreneurs and enterprises operating with complete compliance certainty.')}
            </p>
          </div>
          <div className="shrink-0 flex items-center gap-3">
            ...
            >
              <span>{t('workflow.start_btn', 'Start Free Assessment')}</span>
